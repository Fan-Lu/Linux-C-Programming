...
int e2_open(struct inode *inode, struct file *filp)
{
    ...
    if (devc->mode == MODE1) {
        ...
        devc->count2++;
		pr_info("Device opened in Mode 2 count 2 = %d \n",devc->count2);
    }
    up(&devc->sem1);
    return 0;
}
...
int e2_release(struct inode *inode, struct file *filp)
{
    ...
    if (devc->mode == MODE1) {
        devc->count1--;
		pr_info("Device released in Mode 1 count 1 = %d \n",devc->count1);
        if (devc->count1 == 1)
		{
			wake_up_interruptible(&(devc->queue1));
			pr_info("Wake up any sleeping threads in mode 1 \n");
		} 
			up(&devc->sem2);
    }
    else if (devc->mode == MODE2) {
        devc->count2--;
		pr_info("Device released in Mode 2 count 2 = %d \n",devc->count2);
        if (devc->count2 == 1)
		{
			wake_up_interruptible(&(devc->queue2));
			pr_info("Wake up any sleeping threads in mode 2 \n");
		}
...
static ssize_t e2_read (struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
	...
	ssize_t ret = 0;
	pr_info("Read procedure called \n");
	
    down_interruptible(&devc->sem1);
	if (devc->mode == MODE1) {
	       pr_info("Device read in Mode 1 count 1 = %d \n",devc->count1);
		   up(&devc->sem1);
	   ...
	   ret = count - copy_to_user(buf, devc->ramdisk, count);
		  ...
		  pr_info("Device read in Mode 2 count 2 = %d \n",devc->count2);
          if (*f_pos + count > ramdisk_size) {
             printk("Trying to read past end of buffer!\n");
             up(&devc->sem1);
             return ret;
...
static ssize_t e2_write (struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
	...
	pr_info("Write procedure called \n");
	
	down_interruptible(&devc->sem1);
	if (devc->mode == MODE1) {
		pr_info("Device write in Mode 1 count 1 = %d \n",devc->count1);
		up(&devc->sem1);
        if (*f_pos + count > ramdisk_size) {
		...
		pr_info("Device write in Mode 2 count 2 = %d \n",devc->count2);
        if (*f_pos + count > ramdisk_size) {
            printk("Trying to read past end of buffer!\n");
            up(&devc->sem1);
            return ret;
...
static long e2_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd) {
		case E2_IOCMODE2:
		        pr_info("Device ioctl change to Mode 2 count 1 = %d \n",devc->count1);
				down_interruptible(&(devc->sem1));
				if (devc->mode == MODE2) {
					up(&devc->sem1);
					break;
				...
				if (devc->count1 > 1) {
					while (devc->count1 > 1) {
						pr_info("Thread sleeps until all other instances of device in mode 1 are closed \n");
						up(&devc->sem1);
		...
		case E2_IOCMODE1:
		        pr_info("Device ioctl change to Mode 1 count 2 = %d \n",devc->count2);
				down_interruptible(&devc->sem1);
				if (devc->mode == MODE1) {
				   up(&devc->sem1);
				   break;
				...
				if (devc->count2 > 1) {
				   while (devc->count2 > 1) {
					   pr_info("Thread sleeps until all other instances of device in mode 2 are closed \n");
				       up(&devc->sem1);
