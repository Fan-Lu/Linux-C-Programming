/******************************************************
 * FILE: combiner.c
 * DESCRIPTION:use pthread to do combiner
 *
 * AUTHOR: FAN LU
...
void *mapper(void* param)
{
    ...
    while(1)
    {
        ...
        while(fgets(input, length, pToFileForm))
        {
            ...
            ok = 0;
            for(i=0; i<number_of_buffer; i++)//Full: if there exists any buffer is full!
            {
    ...
    fclose(f);
    fclose(pToFileForm);
    printf("Mapper finish production!\n");
	printf("Feeding data to consumer...\n");
	printf("reducer_output.txt: \n");
...
int main(int argc, char *argv[])
{
    ...
    if (argc < 4) 
	{
        perror("Please Enter Slot size and number of Users");
    }
	else
	{
			number_of_slot = atoi(argv[1]);
			number_of_buffer = atoi(argv[2]);
	}
	...
	/*threads parameters initialization*/
	...
	/*Crete threads*/
    pthread_create(&producer, &attr, mapper, (void *) argv[3]); //producer
    for (i = 0; i < number_of_buffer; i++) 
	{
        pthread_create(&consumer[i], &attr, reducer,(void *)i); //consumer: num determined by command line
    }
