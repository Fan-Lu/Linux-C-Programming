                         ...
                         unsigned int code, int value)
{
	...
	{
        	pr_info("MODE1");
		pr_info("the value of caps lock is %d\n",test_bit(LED_CAPSL,   dev->led));
       
        	if (test_bit(LED_NUML,    dev->led) == 1 && test_bit(LED_CAPSL,   dev->led) == 0)
		{
           
            		kbd->mode=2;
	    		pr_info("MODE2 switched from mode 1");	
	...
	{
        	if (test_bit(LED_NUML, dev->led) == 0)
		{
            		kbd->mode=1;
            		pr_info("MODE1 is switched from mode2");
            		kbd->newleds = (!!test_bit(LED_KANA,    dev->led) << 3) | (!!test_bit(LED_COMPOSE, dev->led) << 3) |
                        (!!test_bit(LED_SCROLLL, dev->led) << 2) |  (!!test_bit(LED_CAPSL,   dev->led) << 1)  |
                        (!!test_bit(LED_NUML,    dev->led));
        	}
		...
		{
			pr_info("MODE2 is still in action");
         		kbd->newleds = (!!test_bit(LED_KANA,    dev->led) << 3) | (!!test_bit(LED_COMPOSE, dev->led) << 3) |
                        (!!test_bit(LED_SCROLLL, dev->led) << 2) | (!test_bit(LED_CAPSL,   dev->led) << 1) |
                        (!!test_bit(LED_NUML,    dev->led));
        	}
